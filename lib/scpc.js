...
const MAP_SIZES = {
  'A4-Portrait': {width: 0.195, height: 0.211}
}

function computeBbox({x, y, taille, orientation, echelle}) {
  const {width, height} = MAP_SIZES[`${taille}-${orientation}`]
  const xMin = x - (width * echelle / 2)
  const xMax = x + (width * echelle / 2)
  const yMin = y - (height * echelle / 2)
  const yMax = y + (height * echelle / 2)
